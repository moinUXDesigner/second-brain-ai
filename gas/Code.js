  ...
  try {
    switch (action) {
      ...
      case "updateTaskStatus":
        ...
        break;
      case "updateTask":
        result = { success: true, data: handleUpdateTask(body) };
        break;
      case "deleteTask":
...
function handleCreateTask(body) {
  ...
  body.area || "",               // D: Area
  body.customArea || "",         // E: Custom_Area ✅ (FIXED)
  body.notes || "",              // F: Notes
  body.projectId || "",          // G: Project_ID
  "",                            // H: Maslow
  "",                            // I: Impact
  "",                            // J: Effort
  recurrenceValue,               // K: Recurrence ✅ (NOW CORRECT COLUMN)
  body.estimatedTime || "",      // L: Estimated_Time
  "",                            // M: Urgency
  "",                            // N: Task_Type
  "",                            // O: Confidence_Score
  "",                            // P: Priority_Score
  ...
  "",                            // S: Source
  "",                            // T: Task_Hash
  "",                            // U: Created_At (or keep empty if auto)
  ""                             // V: Completed_At
];
...
function handleUpdateTask(body) {
  var taskId = String(body.taskId || body.id || "");
  if (!taskId) {
  ...
  for (var i = 1; i < data.length; i++) {
    if (String(data[i][0]) === taskId) {
      ...
      if (body.title !== undefined) taskSheet.getRange(rowIndex, 2).setValue(body.title);
      if (body.type !== undefined) taskSheet.getRange(rowIndex, 3).setValue(body.type);
      if (body.area !== undefined) taskSheet.getRange(rowIndex, 4).setValue(body.area);
      if (body.customArea !== undefined) taskSheet.getRange(rowIndex, 5).setValue(body.customArea);
      if (body.notes !== undefined) taskSheet.getRange(rowIndex, 6).setValue(body.notes);
      if (body.projectId !== undefined) taskSheet.getRange(rowIndex, 7).setValue(body.projectId);
      if (body.recurrence !== undefined) taskSheet.getRange(rowIndex, 11).setValue(body.recurrence);
      if (body.timeEstimate !== undefined) taskSheet.getRange(rowIndex, 12).setValue(body.timeEstimate);
      if (body.urgency !== undefined) taskSheet.getRange(rowIndex, 13).setValue(body.urgency);
      if (body.taskType !== undefined) taskSheet.getRange(rowIndex, 14).setValue(body.taskType);
      if (body.confidence !== undefined) taskSheet.getRange(rowIndex, 15).setValue(body.confidence);
      if (body.priority !== undefined) taskSheet.getRange(rowIndex, 16).setValue(body.priority);
      if (body.fitScore !== undefined) taskSheet.getRange(rowIndex, 17).setValue(body.fitScore);

      if (body.status !== undefined) {
        taskSheet.getRange(rowIndex, 18).setValue(body.status);
        if (body.status === "Done" || body.status === "Deleted") {
          taskSheet.getRange(rowIndex, 22).setValue(new Date());
        } else {
          taskSheet.getRange(rowIndex, 22).setValue("");
        }

        if (todaySheet) {
          var todayData = todaySheet.getDataRange().getValues();
          for (var j = 1; j < todayData.length; j++) {
            if (String(todayData[j][0]) === taskId) {
              todaySheet.getRange(j + 1, 6).setValue(body.status);
              break;
            }
          }
        }
      ...
      // Updated At
      taskSheet.getRange(rowIndex, 21).setValue(new Date());
      updated = true;
      break;
    }
  }
  ...
  // Return updated task from sheet after the update.
  var newData = taskSheet.getDataRange().getValues();
  for (var k = 1; k < newData.length; k++) {
    if (String(newData[k][0]) === taskId) {
      return mapRowToTask(newData[k]);
    }
  }

  return { id: taskId, title: body.title || "", status: body.status || "Pending" };
}
...
function mapRowToTask(row) {
  return {
    ...
    recurrence: String(row[10] || ""),   // ✅ FIXED
    estimatedTime: String(row[11] || ""),// ✅ FIXED

    urgency: String(row[12] || ""),
    taskType: String(row[13] || ""),
    ...
    createdAt: row[20] ? new Date(row[20]).toISOString() : "",
    completedAt: row[21] ? new Date(row[21]).toISOString() : "",
  };
}
